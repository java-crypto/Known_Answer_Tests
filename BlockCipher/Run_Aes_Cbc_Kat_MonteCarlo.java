...
public class Run_Aes_Cbc_Kat_MonteCarlo {

    static byte[][] ctCalculated = new byte[250][128];
    static byte[][] ptCalculated = new byte[250][128];

    public static void main(String[] args) throws Exception {
        TeePrintStream ts = new TeePrintStream(System.out, "AES-CBC_KAT_MonteCarlo_Results.txt", true);
        System.setOut(ts);
        ...
        String[] filenames = { "kat/block_cipher/cbc/CBCMCT128.rsp", "kat/block_cipher/cbc/CBCMCT192.rsp",
        "kat/block_cipher/cbc/CBCMCT256.rsp"};
        for (int algs = 0; algs < filenames.length; algs++) {
            ...
            byte[] IVinit = hexStringToByteArray(KAT_AES_MonteCarlo.iv.get((KAT_AES_MonteCarlo.encryptionStart - 1)));
            ...
            byte[] CIPHERTEXT_cal = aes_cbc_mct_encrypt(PLAINTEXT, KEYinit, IVinit);
            ...
            IVinit = hexStringToByteArray(KAT_AES_MonteCarlo.iv.get((KAT_AES_MonteCarlo.decryptionStart - 1)));
            ...
            byte[] PLAINTEXT_cal = aes_cbc_mct_decrypt(CIPHERTEXT, KEYinit, IVinit);
    ...
    public static byte[] aes_cbc_mct_decrypt(byte[] CIPHERTEXT, byte[] KEYinit, byte[] IVinit) throws Exception {
        int i = 0; // outer loop
        int j = 0; // inner loop
        byte[][] KEY = new byte[101][128];
        byte[][] IV = new byte[1001][128];
        byte[][] CT = new byte[1001][128]; // ciphertext
        byte[][] PT = new byte[1001][128]; // plaintext
        byte[] PTsave = new byte[256]; // nimmt den letzten PT fuer nutzung als neuen iv auf
        // init
        int KEYLENGTH = KEYinit.length * 8;
        KEY[0] = KEYinit;
        IV[0] = IVinit;
        ...
        for (i = 0; i < 100; i++) {
            for (j = 0; j < 1000; j++) {
                if (j == 0) {
                    PT[j] = aes_cbc_decrypt(CT[j], KEY[i], IV[i]);
                    PTsave = CT[j]; // sicherung fuer naechsten iv
                    CT[j + 1] = IV[i];
                } else {
                    IV[i] = PTsave;
                    PT[j] = aes_cbc_decrypt(CT[j], KEY[i], IV[i]);
