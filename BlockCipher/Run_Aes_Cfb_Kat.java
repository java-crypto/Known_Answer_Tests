 ...
 * Funktion: fuehrt den Known Answer Test (KAT) für AES Blockcipher Modus CFB durch
 * Function: performs the Known Answer Test (KAT) for AES blockcipher CFB Mode
 * Beschreibung in / Description in
 * http://java-crypto.bplaced.net/aes-cfb-kat/
 *
...
public class Run_Aes_Cfb_Kat {
    public static void main(String[] args) throws IOException, NoSuchPaddingException, InvalidAlgorithmParameterException, NoSuchAlgorithmException, IllegalBlockSizeException, BadPaddingException, InvalidKeyException {
        TeePrintStream ts = new TeePrintStream(System.out, "AES-CFB_KAT_Results.txt", true);
        System.setOut(ts);
        System.out.println("AES CFB Known Answer Test (KAT)");
        ...
        String[] filenames = { "kat/block_cipher/cfb/CFB128VarKey128.rsp", "kat/block_cipher/cfb/CFB128VarTxt128.rsp",
        "kat/block_cipher/cfb/CFB128GFSbox128.rsp", "kat/block_cipher/cfb/CFB128KeySbox128.rsp",
                "kat/block_cipher/cfb/CFB128VarKey192.rsp", "kat/block_cipher/cfb/CFB128VarTxt192.rsp",
                "kat/block_cipher/cfb/CFB128GFSbox192.rsp", "kat/block_cipher/cfb/CFB128KeySbox192.rsp",
                "kat/block_cipher/cfb/CFB128VarKey256.rsp", "kat/block_cipher/cfb/CFB128VarTxt256.rsp",
                "kat/block_cipher/cfb/CFB128GFSbox256.rsp", "kat/block_cipher/cfb/CFB128KeySbox256.rsp",};
        for (int algs = 0; algs < filenames.length; algs++) {
            ...
            for (int i = 0; i < counterSize; i++) {
                if (KAT_AES.modus.get(i) == "ENCRYPT") {
                    String ciphertextCalculated = bytesToHex(aes_cfb_encrypt(hexStringToByteArray(KAT_AES.plaintext.get(i)), hexStringToByteArray(KAT_AES.key.get(i)), hexStringToByteArray(KAT_AES.iv.get(i))));
                ...
                if (KAT_AES.modus.get(i) == "DECRYPT") {
                    String plainttextCalculated = bytesToHex(aes_cfb_decrypt(hexStringToByteArray(KAT_AES.ciphertext.get(i)), hexStringToByteArray(KAT_AES.key.get(i)), hexStringToByteArray(KAT_AES.iv.get(i))));
        ...
        for (int i = 0; i < nrOfTestfiles; i++) {
            System.out.format("%-42s%5d%8d%8d%n", filename[i], nrTestdata[i], nrTestPassed[i], nrTestFailed[i]);
        }
        ts.close();
    }

    public static byte[] aes_cfb_encrypt(byte[] plaintextByte, byte[] keyByte, byte[] initvectorByte) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, BadPaddingException, IllegalBlockSizeException {
        byte[] ciphertextByte = null;
        SecretKeySpec keySpec = new SecretKeySpec(keyByte, "AES");
        IvParameterSpec ivKeySpec = new IvParameterSpec(initvectorByte);
        Cipher aesCipherEnc = Cipher.getInstance("AES/CFB/NOPADDING");
        aesCipherEnc.init(Cipher.ENCRYPT_MODE, keySpec, ivKeySpec);
        ciphertextByte = aesCipherEnc.doFinal(plaintextByte);
        return ciphertextByte;
    }

    public static byte[] aes_cfb_decrypt(byte[] ciphertextByte, byte[] keyByte, byte[] initvectorByte) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
        byte[] decryptedtextByte = null;
        SecretKeySpec keySpec = new SecretKeySpec(keyByte, "AES");
        IvParameterSpec ivKeySpec = new IvParameterSpec(initvectorByte);
        Cipher aesCipherDec = Cipher.getInstance("AES/CFB/NOPADDING");
